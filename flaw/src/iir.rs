...
impl<const ORDER: usize> SisoIirFilter<ORDER> {
    ...
    pub fn new_interpolated(
        ...
        // Trim values in C that will be unrepresentable
        // c.iter_mut().for_each(|x| if libm::fabsf(*x) < core::f32::EPSILON {*x = 0.0});
        ...
        // The product (1.0 - d) / (1.0 / (1.0 - asum)) must be simplified manually to reduce
        // float error.
        // let csum_desired = (1.0 - d) * (1.0 - asum);
        let csum_desired = 1.0 - asum + (d * asum - d);
        ...
        c.iter_mut().for_each(|v| *v = (*v as f64) * scale_factor);

        let mut c32 = [0.0; ORDER];
        c32.iter_mut()
            .enumerate()
            .for_each(|(i, x)| *x = c[i] as f32);

        Ok(Self::new(&a, &c32, d as f32))
    }
